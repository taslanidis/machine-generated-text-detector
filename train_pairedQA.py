...
def train(args):
    ...
    # Dataset
    dataset = PairedQADataset(human_data_path=args.human_data_path, machine_data_path=args.machine_data_path,
                              tokenizer=tokenizer, paird=args.paired)
    if len(dataset) == 0:
...
def main():
    ...
    parser.add_argument(
        "human_data_path",
        type=str,
        help="Path to the human dataset (e.g., data/my_example_dataset)",
    )
    parser.add_argument(
        "machine_data_path",
        type=str,
        help="path to the machine dataset (e.g., data/my_example_dataset)",
    )
    parser.add_argument(
        "paired",
        type=bool,
        help="Whether or not to use paired human and machine Q&A",
    )
    parser.add_argument(
        "--size",
        type=str,
